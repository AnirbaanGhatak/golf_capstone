...
CAMERA_MATRIX_FRONT = np.array([[1000, 0, 320],
                                [0, 1000, 240],
                                [0,    0,   1]])
CAMERA_MATRIX_SIDE = np.array([[1000, 0, 320],
                               [0, 1000, 240],
                               [0,    0,   1]])
...
class KalmanTracker:
    ...
    def update(self, point):
        # If no new measurement, keep predicting
        if point is None:
        ...
        # Correct with measurement when available
...
def process_frames(front_frame, side_frame, frame_idx, ball_tracker,
                   front_kf, side_kf, calibration, fps):
    ...
    # ---------------------------
    # Kalman filtering commented out
    # if front_club_pos:
    ...
    # Directly append positions without filtering
    # We'll just store them for demonstration
    if front_club_pos:
        front_kf.filtered_path.append(front_club_pos)
    if side_club_pos:
        side_kf.filtered_path.append(side_club_pos)

    ball_tracker.push_position(frame_idx, front_ball_pos, side_ball_pos)

    # Draw the raw (unfiltered) points for the club
    if len(front_kf.filtered_path) > 1:
        for i in range(1, len(front_kf.filtered_path)):
            cv2.line(front_annotated,
                     front_kf.filtered_path[i-1],
                     front_kf.filtered_path[i],
                     (255, 0, 0), 2)

    if len(side_kf.filtered_path) > 1:
        for i in range(1, len(side_kf.filtered_path)):
            cv2.line(side_annotated,
                     side_kf.filtered_path[i-1],
                     side_kf.filtered_path[i],
                     (255, 0, 0), 2)

    # Draw 3D ball trajectory projection
    for i in range(1, len(ball_tracker.positions_3d)):
        ...
        cv2.line(front_annotated, tuple(map(int, p1f)),
                 tuple(map(int, p2f)), (0, 255, 0), 2)
        ...
        cv2.line(side_annotated, tuple(map(int, p1s)),
                 tuple(map(int, p2s)), (0, 255, 0), 2)

    # Calculate and display speeds using the unfiltered path
    ...
    cv2.putText(front_annotated,
                f"Club Speed: {club_speed:.1f} m/s",
...
def main():
    ...
    calibration = GolfCameraCalibration(CAMERA_MATRIX_FRONT,
                                        CAMERA_MATRIX_SIDE,
                                        CAMERA_OFFSET)
    ball_tracker = BallTracker3D(calibration)

    # ---------------------------
    # Kalman trackers created but not actually updated
    front_kf = KalmanTracker()
    side_kf = KalmanTracker()
    # ---------------------------

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    out = cv2.VideoWriter(f"output_{timestamp}.mp4",
    ...
    while True:
        ...
        frame = process_frames(f1, f2, frame_idx,
                               ball_tracker, front_kf,
                               side_kf, calibration, 30)
        out.write(frame)
